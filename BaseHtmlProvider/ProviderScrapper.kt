...
class ProviderScrapper(
    ...
    private suspend fun loadRecursive(url: String, depth: Int): LoadResponse {
        ...
        val seasonDataScript = document.selectSafe(providerId, ProviderHTMLConstants.SELECTOR_SEASON_CONTAINER)
        ...
        if (isMovie) {
            val watchUrl = fixUrlSmart(document.selectSafe(providerId, ProviderHTMLConstants.SELECTOR_WATCH_BUTTONS)?.attr("href"), currentUrl).ifBlank { currentUrl }
    ...
    suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        return runCatching {
            ...
            document.selectSafeList(providerId, ProviderHTMLConstants.SELECTOR_IFRAME_TAG).forEach { el ->

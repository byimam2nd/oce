...
private suspend fun ensureLk21Scope(ctx: Context): Scriptable {
    ...
    synchronized(lk21Lock) {
        ...
        ScriptableObject.putProperty(scope, "navigator", ctx
            .newObject(scope))
        ScriptableObject.putProperty(scope, "location", ctx
            .newObject(scope))
        ScriptableObject.putProperty(scope, "document", ctx
            .newObject(scope))
        ctx.evaluateString(scope, """
            ...
            var atob = function(s) {
                try {
                    var b = java.util.Base64.getDecoder().decode(new java
                        .lang.String(s).getBytes("ISO-8859-1"));
...
suspend fun decryptLk21PlayerUrl(encrypted: String): String? {
    ...
    return runCatching {
        ...
        try {
            ...
            val fn = scope.get("_L",
                scope) as? Function ?: return@runCatching null

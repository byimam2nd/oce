...
class Dramabox : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = response
            ...
            .distinctBy { it.url }
        val result = newHomePageResponse(
        ...
        streams.forEach { s ->
            callback
                .invoke(
                    newExtractorLink(
                        name,
                        "DramaBox ${s.quality?.let {
                            "${it}p"
                        } ?: "Auto"}",
                        s.url!!, ExtractorLinkType.VIDEO
                    ) {
    ...
    private fun cleanTitle(raw: String): String = raw
        .replace(Regex("\\((Sulih Suara|Dub Indo|Indonesian Sub|Sub Indo)\\)", RegexOption.IGNORE_CASE), "")
        .replace(Regex("\\s{2,}"), " ")
        .trim()
    ...
    private suspend fun fetchDramaList(path: String, page: Int): DramaListResponse? = try {
    ...
    private suspend fun fetchDramaDetail(dramaId: String): DramaDetailResponse? = try {

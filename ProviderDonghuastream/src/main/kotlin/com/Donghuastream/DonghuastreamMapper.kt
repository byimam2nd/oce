...
class DonghuastreamMapper(
    ...
    private val configCache: Map<Int, String>
) {
    ...
    fun toSearchResult(element: Element, baseUrl: String? = null): SearchResponse? {
        return runCatching {
            ...
            val cleanRegex = resolveConfig(providerId, DonghuastreamConstants.CONFIG_HREF_CLEAN_REGEXPS, "")
            val cleanReplace = resolveConfig(providerId, DonghuastreamConstants.CONFIG_HREF_CLEAN_REPLACES, "")
    ...
    fun extractMetadata(document: Document, currentUrl: String): MetadataPackage {
        ...
        return MetadataPackage(
            ...
            trailer = document.selectSafe(providerId, LOAD_TRAILER)?.let { if (it.tagName() == "iframe") it.safeExtractImage(ATTR_IMAGE) else it.attrSafe(providerId, listOf(ATTR_HREF)) }
        )
    }
    ...
    fun extractEpisodes(document: Document, currentUrl: String, seasonDataScript: Element?, epItems: org.jsoup.select.Elements, poster: String): List<Episode> {
        ...
        if (episodes.isEmpty()) { episodes.addAll(epItems.mapNotNull { ep -> runCatching { val anchor = ep.selectSafe(providerId, EPISODE_HREF) ?: ep.selectFirst("a") ?: return@runCatching null
                ...
                val isJustNumber = rawName.matches(Regex("""^\d+(\.\d+)?$""")); newEpisode(href) { if (!isJustNumber && rawName.isNotBlank()) this.name = rawName; this.episode = epNum; this.description = ep.selectSafe(providerId, EPISODE_DESC)?.text()?.trim()

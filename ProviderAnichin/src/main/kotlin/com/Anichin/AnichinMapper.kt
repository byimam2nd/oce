...
class AnichinMapper(
    ...
    fun extractMetadata(document: Document, currentUrl: String): MetadataPackage {
        ...
        return MetadataPackage(
            ...
            imdbId = document.selectFirstSafe(providerId, AnichinConstants.SELECTOR_IMDB_EXTERNAL)?.attrSafe(providerId, ATTR_HREF)?.split("/")?.filter { it.startsWith("tt") }?.firstOrNull(),
            tmdbId = document.selectFirstSafe(providerId, AnichinConstants.SELECTOR_TMDB_EXTERNAL)?.attrSafe(providerId, ATTR_HREF)?.split("/")?.lastOrNull()?.toIntOrNull(),

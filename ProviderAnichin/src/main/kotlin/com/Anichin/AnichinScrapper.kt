    ...
    suspend fun search(query: String): List<SearchResponse> {
        ...
        if (isJsonSearch) { return runCatching {
                ...
                for (i in 0 until items.length()) { val item = items.getJSONObject(i)
                    ...
                    results.add(newAnimeSearchResponse(title, finalUrl, if (isTv) TvType.TvSeries else TvType.Movie) { this.posterUrl = pUrl; this.posterHeaders = globalHeaders.toMutableMap().apply { put(AnichinConstants.VAL_REFERER, mainUrl) } })
                }
                results

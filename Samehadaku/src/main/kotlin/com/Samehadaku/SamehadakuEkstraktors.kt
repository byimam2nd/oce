...
suspend fun loadExtractorWithFallback(
    ...
    // Step 2: Try local extractors (Samehadaku currently has none)
    val urlDomain = url.removePrefix("http://").removePrefix("https://").split("/").first().lowercase()
    val matchingExtractors = SamehadakuEkstraktors.list.filter { extractor ->

...
class LayarKaca21Scrapper(
    ...
    private val mapper: LayarKaca21Mapper,
    ...
    suspend fun search(query: String): List<SearchResponse> {
        ...
        if (isJsonSearch) { return runCatching {
                ...
                for (i in 0 until items.length()) { val item = items.getJSONObject(i)
                    ...
                    results.add(api.newAnimeSearchResponse(title, finalUrl, if (isTv) TvType.TvSeries else TvType.Movie) { this.posterUrl = pUrl; this.posterHeaders = globalHeaders.toMutableMap().apply { put(LayarKaca21Constants.VAL_REFERER, mainUrl) } })
                }
                results
    ...
    private suspend fun loadRecursive(url: String, depth: Int): LoadResponse {
        ...
        val seasonDataScript = document.selectSafe(providerId, LayarKaca21Constants.SELECTOR_SEASON_CONTAINER)
        ...
        if (isMovie) {
            val watchUrl = fixUrlSmart(document.selectSafe(providerId, LayarKaca21Constants.SELECTOR_WATCH_BUTTONS)?.attr("href"), currentUrl).ifBlank { currentUrl }
            return api.newMovieLoadResponse(metadata.title, url, type, episodeDataUrlPattern.replace("{url}", watchUrl)) { 
                ...
                this.recommendations = recommendations; this.comingSoon = metadata.statusText?.let { st -> resolveConfig(providerId, LayarKaca21Constants.STR_COMING_SOON, "").split(",").any { st.contains(it, true) } } ?: false
    ...
    suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        return runCatching {
            ...
            document.selectSafeList(providerId, LayarKaca21Constants.SELECTOR_IFRAME_TAG).forEach { el ->
            ...
            coroutineScope {
                allPossibleLinks.filter { it.first.isNotBlank() }.map { (raw, label) -> async { runCatching {
                    ...
                    if (!okDirect) {
                        val refererMode = resolveConfig(providerId, CONFIG_HOOK_REFERER_PLAYER, LayarKaca21Constants.STR_REFERER_MODE_CURRENT)
                        val refererForPlayer = if (refererMode == LayarKaca21Constants.STR_REFERER_MODE_SERIES) "$seriesUrl/" else currentUrl

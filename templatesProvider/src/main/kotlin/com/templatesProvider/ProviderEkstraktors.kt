...
open class Odnoklassniki : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val videoReq = app.get(embedUrl).text
            .replace("\\&quot;", "\"")
            .replace("\\\\", "\\")
...
open class Vidguardto : ExtractorApi() {
            ...
            try {
                ...
                ExtractorHelpers.extractVideoFromMeta(res.text)?.let { directM3u8 ->
                    if (directM3u8.contains(".m3u8")) {
                        callback.invoke(newExtractorLink("${this.name} Direct", "${this.name} Direct", directM3u8, ExtractorLinkType.M3U8) { this.referer = mainUrl })
                    }
                }
            }
        }
    ...
    private fun runJS2(hideMyHtmlContent: String): String {
        ...
        val r = Runnable {
            ...
            try {
                ...
                } else Context.toString(svgObject)
            } catch (_: Exception) {
    ...
    private fun getEmbedUrl(url: String): String {
        return url.takeIf { it.contains("/d/") || it.contains("/v/") }?.replace("/d/", "/e/")?.replace("/v/", "/e/") ?: url
    }
    data class SvgObject(val stream: String, val hash: String)
}

class Vidguardto1 : Vidguardto() { override val mainUrl = "https://bembed.net" }
class Vidguardto2 : Vidguardto() { override val mainUrl = "https://listeamed.net" }
class Vidguardto3 : Vidguardto() { override val mainUrl = "https://vgfplay.com" }
...
class Dailymotion : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val embedUrl = if (url.contains("/embed/") || url.contains("/video/")) url 
                      else if (url.contains("geo.dailymotion.com")) "$baseUrl/embed/video/${url.substringAfter("video=")}"
                      else return
        val id = URI(embedUrl).path.substringAfter("/video/").takeIf { it.matches("^[kx][a-zA-Z0-9]+$".toRegex()) } ?: return

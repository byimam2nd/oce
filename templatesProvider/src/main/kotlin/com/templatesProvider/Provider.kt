...
/**
 * ULTIMATE HTML SCRAPING TEMPLATE - VERSION 3.0 (ULTIMATE)
 * 
 * Fitur Lengkap:
 * - Multi-URL (Main, Series, Search)
 * - Flexible Search Pattern & Configurable Depth
 * - Episode Sorting Toggle (Reverse)
 * - Download Links Support
 * - Global Headers & User-Agent (Anti-Bot)
 * - Show Status & Quality Detection
 */
...
open class TemplatesProvider : MainAPI() {
    ...
    open var seriesUrl = "https://example.com"
    open var searchUrl = "https://example.com"

    override var name = "Templates Provider"
    ...
    // --- Advanced Config ---
    open var searchPathPattern = "{baseUrl}/page/{page}/?s={query}"
    open var searchPageLimit = 2
    open var reverseEpisodes = true
    open var globalHeaders = mapOf(
    ...
    companion object {
        const val TAG = "TemplatesProvider"
        const val DEFAULT_TIMEOUT = 10000L

        // --- Search & Home Selectors ---
        val SEARCH_ITEMS = listOf("article", "div.listupd > article", "div.listupd div.bs", "div.animposx", "article figure", ".listupd .bsx", ".item", ".post-item")
        ...
        val LOAD_TITLE = listOf("h1.entry-title", "h1.title", ".entry-title", ".post-title", "div.movie-info h1", "div.infox h1", "h1[itemprop=headline]")
        val LOAD_POSTER = listOf(".thumb img", ".poster img", "div.bigcontent img", "img.ts-post-image", "meta[property=og:image]", ".wp-post-image")
        val LOAD_BANNER = listOf(".banner img", ".backdrop img", "meta[property=og:image:secure_url]")
        val LOAD_DESC = listOf(".entry-content", ".description", ".plot", "div.desc p", "div.description p", "div.sinopsis p", "div.entry-content p", "meta[name=description]")
        ...
        val LOAD_RATING = listOf(".rating", ".imdb-rating", ".score", ".rating-value", "div.info-tag strong", "div.rating strong")
        ...
        val LOAD_TRAILER = listOf("iframe[src*='youtube.com/embed/']", "a[href*='youtube.com/watch']", "a[href*='youtu.be']", "ul.action-left > li:nth-child(3) > a", "div.trailer iframe")
        val LOAD_RECOMMEND = listOf(".recommendations article", ".related-post article", ".similar article", ".related article", "li.slider article", "div.related-series article")
        
        // --- Episode Selectors ---
        val EPISODE_ITEMS = listOf(".eplister li", ".ep-list li", ".list-episode li", ".list-ep li", "div.lstepsiode ul li", "ul#daftarepisode > li")
        ...
        val LINK_OPTIONS = listOf("select.mirror option", ".server-list li", ".player-option", ".dropdown-menu li", "div#downloadb li", "option[data-index]", ".mobius > .mirror > option", "ul#player-list a")
        val LINK_IFRAME_GLOBAL = listOf("iframe")
        val DOWNLOAD_ITEMS = listOf(".dl-wrapper a", ".download-link a", ".box-download a", ".dl-list a", "a.btn-download")
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        return runCatching {
            ...
            val document = executeWithRetry {
                rateLimitDelay()
                app.get("$baseUrl/${request.data}$page", timeout = DEFAULT_TIMEOUT, headers = globalHeaders).documentLarge
            }
            ...
            (1..searchPageLimit).map { page ->
                async {
                    runCatching {
                        ...
                        val document = app.get(url, timeout = DEFAULT_TIMEOUT, headers = globalHeaders).documentLarge
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val description = document.selectSafe(LOAD_DESC)?.let { if (it.tagName() == "meta") it.attr("content") else it.text() }?.trim() ?: ""
        ...
        val imdbId = document.selectFirst("a[href*='imdb.com/title/']")?.attr("href")?.split("/")?.filter { it.startsWith("tt") }?.firstOrNull()
        val tmdbId = document.selectFirst("a[href*='themoviedb.org/']")?.attr("href")?.split("/")?.lastOrNull()?.toIntOrNull()

        val trailer = document.selectSafe(LOAD_TRAILER)?.let { if (it.tagName() == "iframe") it.attr("src") else it.attr("href") }
        ...
        return if (isMovie) {
            ...
            newMovieLoadResponse(title, url, type, dataUrl) {
                ...
                addTrailer(trailer)
                addActors(actors)
                addMalId(tracker?.malId); addAniListId(tracker?.aniId?.toIntOrNull()); addImdbId(imdbId); addTMDbId(tmdbId?.toString())
            }
        } else {
            var episodes = document.selectSafeList(EPISODE_ITEMS).mapNotNull { ep ->
                runCatching {
                    ...
                    newEpisode(href) {
                        this.name = ep.selectSafe(EPISODE_TITLE)?.text()?.ifBlank { "Episode ${epNum ?: "?"}" } ?: "Episode ${epNum ?: "?"}"
                        this.episode = epNum
                        this.description = ep.selectSafe(EPISODE_DESC)?.text()?.trim()
                        this.runTime = ep.selectSafe(EPISODE_TIME)?.text()?.filter { it.isDigit() }?.toIntOrNull()
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        return runCatching {
            val document = executeWithRetry { rateLimitDelay(); app.get(data, timeout = DEFAULT_TIMEOUT, headers = globalHeaders).documentLarge }
                            ...
                            runCatching {
                                ...
                                val decodedUrl = if (serverData.safeIsBase64()) {
                                    runCatching { 
                                        ...
                                        if (decoded.contains("iframe")) Jsoup.parse(decoded).selectFirst("iframe")?.attr("src") else decoded
                                    }.getOrNull() ?: serverData
                                } else serverData

                                if (!decodedUrl.isNullOrBlank()) {
                                    val fixedUrl = decodedUrl.safeHttpsify()
                                    val success = runCatching { loadExtractorWithFallback(fixedUrl, data, subtitleCallback, callback) }.getOrDefault(false)
                                    if (!success && (fixedUrl.contains(".mp4") || fixedUrl.contains(".m3u8") || fixedUrl.contains(".mkv"))) {
                                        MasterLinkGenerator.createLink(name, fixedUrl, data, fixedUrl.safeGetQuality())?.let { callback(it) }
                                    }
                                }
                            }
                        }
    ...
    private fun Element.safeExtractImage(attributes: List<String>): String {
        return try {
